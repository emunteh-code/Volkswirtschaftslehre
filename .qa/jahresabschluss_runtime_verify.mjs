...
import { CHAPTERS } from '../jahresabschluss/js/data/chapters.js';

const chromePath = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const baseUrl = 'http://127.0.0.1:4181';
const consentKey = 'jahresabschluss_consent_v1';
...
async function openModulePage(browser) {
  ...
  await page.goto(`${baseUrl}/jahresabschluss/index.html?qa=1`, { waitUntil: 'domcontentloaded' });
...
try {
  ...
  await page.screenshot({ path: '.qa/jahresabschluss-home.png', fullPage: true });
  ...
  await navigateConcept(page, 'anlagevermoegen', 'theorie');
  await page.screenshot({ path: '.qa/jahresabschluss-anlagevermoegen.png', fullPage: true });
  await navigateConcept(page, 'umlauf_waren_ust', 'aufgaben');
  await page.screenshot({ path: '.qa/jahresabschluss-umlauf-waren-ust.png', fullPage: true });

  await page.evaluate(() => window.__showFullExamSelect());
  await page.waitForTimeout(900);
  await page.screenshot({ path: '.qa/jahresabschluss-exams.png', fullPage: true });
  ...
  await page.waitForTimeout(900);
  await page.screenshot({ path: '.qa/jahresabschluss-full-exam-1.png', fullPage: true });
