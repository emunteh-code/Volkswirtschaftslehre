...
async function openModulePage(browser) {
  ...
  await page.goto(`${baseUrl}/recht/index.html?qa=1`, { waitUntil: 'domcontentloaded' });
...
try {
  ...
  await page.screenshot({ path: '.qa/recht-home.png', fullPage: true });
  ...
  await navigateConcept(page, 'willenserklaerung', 'theorie');
  await page.screenshot({ path: '.qa/recht-willenserklaerung.png', fullPage: true });
  await navigateConcept(page, 'schadensersatz', 'aufgaben');
  await page.screenshot({ path: '.qa/recht-schadensersatz.png', fullPage: true });

  await page.evaluate(() => window.__showFullExamSelect());
  await page.waitForTimeout(900);
  await page.screenshot({ path: '.qa/recht-exams.png', fullPage: true });
  ...
  await page.waitForTimeout(900);
  await page.screenshot({ path: '.qa/recht-full-exam-1.png', fullPage: true });
