...
function renderLegalSchemaRP(eq) {
  return String(eq || "")
  ...
  if (rpF) {
    if (d && d.formeln && d.formeln.length) {
      ...
      rpF.innerHTML = d.formeln.map((f, i) => {
        const eqHtml = isLegalSchemaRP(f.eq)
          ? `<div class="legal-schema">${renderLegalSchemaRP(f.eq)}</div>`
          : f.eq;
        return `<div class="rp-formula" title="Klicken zum Kopieren" role="button" tabindex="0" data-formula-idx="${i}">
