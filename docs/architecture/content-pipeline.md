...
- **`module-content.js`:** **`mikro1` / `mikro2`** are present (`module-content-parity-cleanup-pass-1.md`); **`internationale-wirtschaftsbeziehungen`** has a block under the **quoted** slug key (`iwb-status-parity-audit-pass-1.md`) — access via `getModuleContent(slug)` only.
...
| **`module-content.js` parity for micro modules** | **`mikro1`** and **`mikro2`** entries exist. **`mikro2`** now has an official corpus and a partial provenance manifest, but still needs full 20-lecture reconstruction before source-complete parity; current status: `docs/audits/mikro2-official-source-ingest-pass-1.md`. **`internationale-wirtschaftsbeziehungen`** has a narrative block (quoted key); see `docs/audits/iwb-status-parity-audit-pass-1.md`. |
