...
export function renderDashboard(onNavigate) {
  ...
  const avgAccuracy = stats.filter((s) => s.accuracy !== null).reduce((a, s, _, arr) => a + s.accuracy / arr.length, 0);
  const weak = stats.filter((s) => s.accuracy !== null && s.accuracy < 0.6).sort((a, b) => a.accuracy - b.accuracy);
...
<p style="color:var(--muted);font-size:12px;margin-top:8px;margin-bottom:0">Lokal gespeicherte Fehler (sofern Übungen sie protokollieren).</p>
</div>`;
  html += `<div class="dash-section" style="margin-bottom:16px">
