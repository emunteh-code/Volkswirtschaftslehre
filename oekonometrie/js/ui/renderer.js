...
const MATH_SCRIPT_CHARS = '₀₁₂₃₄₅₆₇₈₉ₐₑₒₓₘₙₚᵢⱼᵣᵤᵥₖₜₛ*′';
const MATH_SUPERSCRIPT_CHARS = '⁰¹²³⁴⁵⁶⁷⁸⁹';
const MATH_GREEK_CHARS = 'λμωπΔεαβρσθΩūȳ';
const MATH_JOINER_REGEX = /^[\s0-9.,%()|=<>≤≥+\-−·/∂→↔*^:]+$/u;
const MATH_TRAILING_NUMBER_REGEX = /^\s*(?:=|<|>|≤|≥)\s*\d+(?:[.,]\d+)?%?/u;
const MATH_RANGE_PATTERNS = [
  /€/gu,
  /\d+(?:[.,]\d+)?\s*€/gu,
  /\b(?:GRS|GRTS|MR|MC|AC|AVC|CV|EV|DWL|KR|PR|SE|EE|MZB|IK|BEO|OLS|GLS|WLS|HAC|VIF|TSS|ESS|SSR|DW)\b/gu,
...
function decodeTextEntitiesInPlace(root) {
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (!parent || parent.closest('script, style, textarea, input, select, option, pre, code, .r-code-block')) {
...
function decorateSemanticMath(root) {
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (parent.closest('mjx-container, script, style, textarea, input, select, option, canvas, svg, pre, code, .r-code-block, .math-block, .math-inline, .math-semantic')) {
        return NodeFilter.FILTER_REJECT;
      }
