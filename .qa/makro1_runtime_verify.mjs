...
import { CHAPTERS } from '../makro1/js/data/chapters.js';

const chromePath = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const baseUrl = 'http://127.0.0.1:4181';
const consentKey = 'makro1_consent_v1';
...
async function openModulePage(browser) {
  ...
  await page.goto(`${baseUrl}/makro1/index.html?qa=1`, { waitUntil: 'domcontentloaded' });
...
try {
  ...
  await page.screenshot({ path: '.qa/makro1-home.png', fullPage: true });
  ...
  await navigateConcept(page, 'islm', 'graph');
  await page.screenshot({ path: '.qa/makro1-islm.png', fullPage: true });
  await navigateConcept(page, 'phillips', 'graph');
  await page.screenshot({ path: '.qa/makro1-phillips.png', fullPage: true });

  await page.evaluate(() => window.__showFullExamSelect());
  await page.waitForTimeout(800);
  await page.screenshot({ path: '.qa/makro1-exams.png', fullPage: true });
  ...
  await page.waitForTimeout(900);
  await page.screenshot({ path: '.qa/makro1-full-exam-1.png', fullPage: true });
